...
class Pessoa{
    ...
    public function atualizar($idcadastro,$nome,$telefone,$email)
    {
        $pdos = Connect::getConnection();
        $res = $pdos->prepare("UPDATE cadastro SET nome = :nome, telefone = :telefone, email = :email WHERE idcadastro = :id");
        $res->bindParam(':nome', $nome);
        $res->bindParam(':telefone', $telefone);
        $res->bindParam(':email', $email);
        $res->bindParam(':id', $idcadastro);
